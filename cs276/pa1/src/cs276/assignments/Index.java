...
public class Index {
	...
	static Map<Integer, Pair<Long, Integer>> postingDict 
		= new TreeMap<Integer, Pair<Long, Integer>>();
	// Doc name -> doc id dictionary
	static Map<String, Integer> docDict
		= new TreeMap<String, Integer>();
	// Term -> term id dictionary
	static Map<String, Integer> termDict
		= new TreeMap<String, Integer>();
	// Block queue
	static LinkedList<File> blockQueue
		= new LinkedList<File>();
	...
	public static void main(String[] args) throws IOException {
		...
		for (File block : dirlist) {
			 ...
			 * Your code here
			 */
			Map<Integer, Set<Integer>> postings = new TreeMap<Integer, Set<Integer>>();
				...
				while ((line = reader.readLine()) != null) {
					...
					for (String token : tokens) {
						...
						if (!termDict.containsKey(token)) {
							...
							postings.put(wordIdCounter, new TreeSet<Integer>());
							wordIdCounter++;
						} else if (!postings.containsKey(termDict.get(token))) {
							postings.put(termDict.get(token), new TreeSet<Integer>());							
						}
			 ...
			 * Your code here
			 */
			for (Map.Entry<Integer, Set<Integer>> entry : postings.entrySet()) {
				writePosting(bfc.getChannel(), new PostingList(entry.getKey(), new ArrayList<Integer>(entry.getValue())));
			}
		...
		while (true) {
			...
			while (bf1Posting != null || bf2Posting != null) {
				...
				} else if (bf1Posting != null) {
					writePosting(mf.getChannel(), bf1Posting);
					bf1Posting = index.readPosting(bf1.getChannel());						
				} else { //bf2Posting != null
					writePosting(mf.getChannel(), bf2Posting);
					bf2Posting = index.readPosting(bf2.getChannel());						
				}
			}
